#TODO
#----------------------------------------------------------------------------------
#TODO IGOFF信号が一瞬しか受信できないため、信号をラッチする処理を追記(追加済み。動作確認未)→上手く動作せず
#TODO 今まではCANの情報をすべて保存するしか無かったが必要な情報だけを保存する処理を追加
#TODO キーボード入力をノンブロッキングで受け取って終了できる処理を追加
#TODO エンジン停止を判別することが現状できないため、どうすればいいか検討→RPMの前回値を記憶しておいて10秒間まったく同じ値だったらエンジン停止判定
#TODO OBDカプラから電力をとれるように回路を作成。
#----------------------------------------------------------------------------------
...
import datetime
import traceback
#import msvcrt#wwindows用のモジュールっぽいため使えず。。。
...
while (msg != None) and (rpm != 0):#CAN信号途絶もしくはエンジン停止したらロギング停止
    msg = can0.recv(10.0)
    ...
    if len(msg_list) > 3:#エンジン停止後のECUスリープ前にメッセージがタイムスタンプのみ保存され、msg_list[3]の処理が要素外アクセスエラーが発生するため回避用
        if msg_list[3] == "0158":
            ...
            gas_pedal_open = cal_hex2dec_1byte(msg_list[9])*100/255#アクセル開度情報(最大FFなので、アクセル開度100%を255としている。)
        ...
        elif msg_list[3] == "0191":
            select_lever =cal_hex2bin_1byte(msg_list[8])#セレクトレバー情報(5bit:L,4bit:S,3bit,D,2bit:R,0bit:P)
        if (distance != 0) and (fi_total != 0):
            fuel_economy = distance/(fi_total/1000)#燃料噴出量をℓに変換したうえで燃費を求める
        print("\r car_speed: %lf [km/h] \n rpm : %lf [rpm] \n distance: %lf [km] \n gas_pedal_open: %lf [%%] \n water_temp: %lf [℃] \n fuel_remain : %lf [%%] \n FI_total: %lf [ml] \n FI_diff:%lf [ml] \n fuel_economy: %lf [km/l]\n IG_OFF_FLAG_RATCH:%lf \033[9A" %(car_speed, rpm, distance, gas_pedal_open, water_temp, fuel_remain, fi_total, fi_diff, fuel_economy,IG_OFF_FLAG_RATCH), end =" ")#必要な情報を表示
...
if (msg is None) or (rpm == 0):#CAN信号途絶もしくはエンジン停止したらロギング停止
#if (msg is None):
